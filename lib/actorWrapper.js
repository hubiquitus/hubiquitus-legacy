...
function sendWrapper(_this, actorInstance, message, cb) {
  if (cb) {
    _this.send(message.actor, message, message.timeout, function (err, res) {
      if (err) {
        var response = actorInstance.buildResult(res.to, message.msgid, codes.TECH_ERROR, err);
    ...
    _this.send(message.actor, message);
  }
}
...
function ActorWrapper(id, legacyActor, logger) {
  ...
  legacyActor.send = function(message, cb) {
    sendWrapper(_this, legacyActor, message, cb);
  };
...
module.exports = function (id, legacyActor) {
  return new ActorWrapper(logger, id, legacyActor);
}
